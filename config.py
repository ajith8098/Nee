# Bot Token from @BotFather
BOT_TOKEN = "6750957059:AAFvd3mez21vumWuV8jMaG3aOBW55_eGTKM"

# API ID and API Hash from my.telegram.org
API_ID = 123456  # Replace with your API ID
API_HASH = "your_api_hash"  # Replace with your API Hash

# ID of the Telegram channel to monitor (e.g., -1001234567890)
CHANNEL_ID = -1001870015374

# ID of the logs channel to send ban details (e.g., -1009876543210)
LOGS_CHANNEL_ID = -1001870015374

# Bot Owner's Telegram ID (optional, for direct notifications)
OWNER_ID = 5821871362

# Path to save the ban list
BAN_LIST_FILE = "ban_list.txt"
